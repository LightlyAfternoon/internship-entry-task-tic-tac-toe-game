...
namespace mobibank_test.mobi_test_test_project.test
{
    public class SessionControllerUnitTest
    {
        ...
        {
            var session = new Session(1L, 2L);
            var mockSessionService = new Mock<ISessionService>();
            var mockFieldCellService = new Mock<IFieldCellService>();
            var controller = new SessionController(mockSessionService.Object, mockFieldCellService.Object);

            mockSessionService.Setup(x => x.Add(session)).Returns(new Session(1L, session));

            Assert.Equal(((JsonHttpResult<Session>)Results.Json(new Session(1L, session))).Value,
        ...
        public void UpdateSessionTest()
        {
            ...
            mockSessionService.Setup(x => x.FindById(2L)).Returns(new Session(2L, session));
            session.PlayerYId = 3L;
            mockSessionService.Setup(x => x.Update(2L, session)).Returns(new Session(2L, session));

            Assert.Equal(((JsonHttpResult<Session>)Results.Json(new Session(2L, session))).Value,
        ...
        {
            var session = new Session(1L, 1L, 2L);
            var move = new FieldCell(0, 1, 1L, session.Id);
            var mockSessionService = new Mock<ISessionService>();
            var mockFieldCellService = new Mock<IFieldCellService>();
            var controller = new SessionController(mockSessionService.Object, mockFieldCellService.Object);
        ...
        public void UpdateSessionMoveTest()
        {
            ...
            mockSessionService.Setup(x => x.FindById(1L)).Returns(session);
            mockFieldCellService.Setup(x => x.FindById(2L)).Returns(new FieldCell(2L, move));
            move.Y = 2;
            mockFieldCellService.Setup(x => x.Update(session.Id, move)).Returns(new FieldCell(2L, move));

            Assert.Equal(((JsonHttpResult<FieldCell>)Results.Json(new FieldCell(2L, move))).Value,
